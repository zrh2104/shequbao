...
import com.shequbao.model.FoodInfo;
import com.shequbao.model.PingCheInfo;
import com.shequbao.model.ShopInfo;
import com.shequbao.model.SignInfo;
import com.shequbao.tools.CommentsInfo;
...
public class MyJson {
	...
	private ArrayList<PingCheInfo> PingCheList = new ArrayList<PingCheInfo>();
	...
	// 解析拼车列表
		public List<PingCheInfo> getPingCheList(String value) {
			List<PingCheInfo> list = null;
			try {
				JSONArray jay = new JSONArray(value);
				list = new ArrayList<PingCheInfo>();
				for (int i = 0; i < jay.length(); i++) {
					JSONObject job = jay.getJSONObject(i);
					PingCheInfo info = new PingCheInfo();
					info.setSuser(job.getString("suser"));
					info.setIusericon(job.getString("iusericon"));
					info.setSstarttime(job.getString("sstarttime"));
					info.setSstartdest(job.getString("sstartdest"));
					info.setSenddest(job.getString("senddest"));
